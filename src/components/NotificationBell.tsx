...
function NotifSheet({ notifications, unreadCount, markAllRead, markRead, onClose }: {
      ...
      <div className="ht-modal">
        ...
        <div style={{ padding:'0 16px 32px', maxHeight:'65vh', overflowY:'auto' }}>
          {notifications.length === 0
            ...
            : notifications.map(n => <NotifItem key={n.id} n={n} onRead={() => markRead(n.id)} />)
          }
        </div>
      </div>
    </>
...
interface Props {
  userId?: string
  householdId?: string
  /** Si true, muestra solo el panel (sin botón de campana) */
  forceOpen?: boolean
  onClose?: () => void
}

export default function NotificationBell({ userId, householdId, forceOpen, onClose }: Props) {
  ...
  // Modo embebido: solo muestra el panel (usado desde BottomNav)
  if (forceOpen) {
    return (
      <NotifSheet
  ...
  return (
    <>
      ...
      >
        ...
        {unreadCount > 0 && (
          <div style={{ position:'absolute', top:-2, right:-2, width:16, height:16, borderRadius:9999, background:'var(--ht-rose)', border:'2px solid white', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:900, color:'white' }}>
